...
constexpr double kDrivetrainEncoderRatio =
    1.0;  /// 20.0; //TODO (jasmine): Encoders are 1:1 to the wheel; allow high
          /// gear to be used.
constexpr double kDrivetrainHighGearRatio = 1.0 / 9.0;
constexpr double kDrivetrainLowGearRatio = 1.0 / 20.0;
const bool kDrivetrainClutchTransmission =
    true;  // false; // We want to currently just assume that the shifters shift
           // immediately/
// End constants
...
class DrivetrainLoop {
 public:
  ...
  // TODO (jasmine): Send everything to a safe state, usually 'motors off',
