...
void driveTrainValues(float stickLeftInput, float stickRightInput,
		float kJoystickChangeThreshold) {
...
void deadzone() {
	...
	if ((useright >= -kDeadzoneThreshold) && (useright <= kDeadzoneThreshold)) {
		useright = 0;
	}
}

void runDrivetrain(float stickLeftInput, float stickRightInput,
		RobotDrive *drivetrain, float thresh, bool straight) {
	// Drivetrain code

	driveTrainValues(stickLeftInput, stickRightInput, thresh);
	deadzone();
	//drivetrain->TankDrive(useleft * 128 + 128, useright * 128 + 128);
...
void runDrivetrain(float stickLeftInput, float stickRightInput,
		RobotDrive *drivetrain, bool straight) {
	if (straight)
		stickLeftInput = stickRightInput;
	driveTrainValues(stickLeftInput, stickRightInput, 0.2);
	deadzone();
	//drivetrain->TankDrive(useleft * 128 + 128, useright * 128 + 128);
	drivetrain->TankDrive(useleft, useright);
	// Inverted and negated the values to make the back of the robot the "front"
...
void runDrivetrainShift(float stickLeftInput, float stickRightInput,
		RobotDrive *drivetrain, float thresh, Solenoid *gearUp,
		Solenoid *gearDown, Encoder *leftEncoder, Encoder *rightEncoder) {
	driveTrainValues(stickLeftInput, stickRightInput, thresh);
	deadzone();
	...
	if (((useleft > outsideValue && leftEncoder->GetRate() < insideRate)
			...
			((useright > outsideValue && rightEncoder->GetRate() < insideRate)
					|| ((useright < -outsideValue)
							&& (rightEncoder->GetRate() > -insideRate)))) {
		//gearUp->Set(false);
		//gearDown->Set(true);
		counter++;
	} else {
