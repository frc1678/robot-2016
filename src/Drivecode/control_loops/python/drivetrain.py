...
#Jasmine: modified to take citrus's constants
class Drivetrain(control_loop.ControlLoop):
  def __init__(self, name="Drivetrain", left_low=True, right_low=True):
    ...
    self.m = 68 #5.8 + 5.8 kg 65.75 + 1.25
    # Radius of the robot, in meters (from last year).
    self.rb = 0.9603 / 2.0 #Diameter: 24.5 inches
    # Radius of the wheels, in meters.
    self.r = 0.0508 #Diameter: 
    # Resistance of the motor, divided by the number of motors.
    self.R = 12.0 / self.stall_current / 2 #6-cim robots
    # Motor velocity constant
    self.Kv = ((self.free_speed / 60.0 * 2.0 * numpy.pi) /
               (12.0 - self.R * self.free_current))
    # Torque constant
    self.Kt = self.stall_torque / self.stall_current
    # Gear ratios
    self.G_const = 18.0 / 44.0 * 18.0 / 60.0 #4.17 and 11.03  //probably 1/ cf Megan
